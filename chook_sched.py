...
COOKIES = dict(wgcwx_session_key='870f565c81d9199e54bcaa1a3cb8c7a8')
##creds = ServiceAccountCredentials.from_json_keyfile_name('client_secret.json', SCOPE)
creds = ServiceAccountCredentials.from_json_keyfile_name('chook2_creds.json', SCOPE)
...
def getBase(arenaName, baseNo):
    baseDict = {'Abbey': {1: 'South', 2: 'North'}, 'Cliff': {1: 'South', 2: 'North'}, 'Ensk': {1: 'North', 2: 'South'},
            ...
            'Westfield': {1: 'West', 2: 'East'},'Safe Haven': {1: 'West', 2: 'East'}}
...
def prettyList(input, enemy=False):
    ...
    front = 'Basic' if input['front_id'] == "iron_age_sg_league1" else 'Advanced'
...
while(True):
    ...
    #hour = int(time.strftime('%H', nzTime)) - 8 
    funk = False
    while funk is False:
        battles = requests.get('https://asia.wargaming.net/globalmap/game_api/clan/2000011004/battles', cookies=COOKIES)
        try:
...
# 12 col L 17 col Q 1 indexed
    ...
    updateString = time.strftime('Last updated: %a %d/%m/%y at %H:%M:%S NZSTa', nzTime)
    try:
